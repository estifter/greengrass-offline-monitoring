...
from src.publish_config import ConfigurationPublisher
...
if __name__ == "__main__":
    ...
    container_management = ContainerManagement(
        ipc_client, docker_client, configuration_handler
    )
    container_management.subscribe_to_configuration_updates()
    container_management.manage_postgresql_container(
        configuration_handler.get_configuration()
    )
    config_publisher = ConfigurationPublisher()
    config_publisher.listen_for_config_requests(ipc_client, container_management)
