...
class ComponentConfiguration:
    ...
    def __init__(
        self,
        config_response: GetConfigurationResponse,
        secret_reponse: GetSecretValueResponse,
    ):
        self.__host_volume = DEFAULT_HOST_VOLUME
        self.__host_port = DEFAULT_HOST_PORT
        self.__container_name = DEFAULT_CONTAINER_NAME
        self.__container_network = DEFAULT_NETWORK_NAME
        self.__db_username = ""
        self.__db_password = ""
    ...
    def _set_container_config(self, config_response: GetConfigurationResponse):
        ...
        if container_config.get(CONTAINER_NETWORK_KEY):
            self.__container_network = container_config[CONTAINER_NETWORK_KEY]

    def _set_configuration_files(self, config_response):
        component_config = config_response.value
        if POSTGRES_SERVER_CONFIGURATION_FILES_KEY not in component_config:
            return
        server_configuration_files = component_config[
            POSTGRES_SERVER_CONFIGURATION_FILES_KEY
        ]
        ...
        for conf_file, file_path in server_configuration_files.items():
            ...
            if not conf_file_abs_path.is_file():
                logging.warning(
                    "{} will not be used as is not a valid file path.".format(
                        conf_file_abs_path
                    )
                )
                continue
    ...
    def _valid_password(self, password):
        if len(password) < 16:
            logging.warning(
                "The length of the postgresql password should be at least 16 character long"
            )
            return False
        if not re.search("[0-9]", password):
            logging.warning(
                "Provided postgresql password should contain at least one digit"
            )
            return False
        if not re.search("[a-z]", password):
            logging.warning(
                "Provided postgresql password should contain at least one lowercase letter"
            )
            return False
        if not re.search("[A-Z]", password):
            logging.warning(
                "Provided postgresql password should contain at least one uppercase letter"
            )
            return False
        if not re.search("[^a-zA-Z0-9]", password):
            logging.warning(
                "Provided postgresql password should contain at least one special character"
            )
            return False
        return True
    ...
    def get_container_network(self):
        "Returns docker container network"
