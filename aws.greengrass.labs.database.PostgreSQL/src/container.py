...
class ContainerManagement:
    def __init__(
        ...
        config_handler: ComponentConfigurationIPCHandler,
    ) -> None:
    ...
    def subscribe_to_configuration_updates(self):
        ...
        def __on_stream_error_event(error: Exception) -> bool:
            logging.error(
                "Exception occurred in the stream while subscribing to the configuration updates",
                exc_info=error,
            )
    ...
    def _set_container(self, configuration: ComponentConfiguration):
        if not self.postgresql_container:
            try:
                self.postgresql_container = self.docker_client.containers.get(
                    configuration.get_container_name()
                )
            except docker.errors.NotFound as not_found:
                logging.debug(
                    "Exception while getting the container: ", not_found.explanation
                )
    ...
    def manage_postgresql_container(self, configuration: ComponentConfiguration):
        ...
        logging.info(
            "Creating a new docker container: %s as the configuration changed",
            configuration.get_container_name(),
        )
    ...
    def _stop_container(self):
        if not self.postgresql_container:
            return
        logging.info(
            "Stopping the docker container : {}-{}".format(
        ...
        try:
            ...
            logging.debug(
                "Could not stop the container: {}-{} as it does not exist : {}".format(
                    self.postgresql_container.name,
                    self.postgresql_container.id,
                    e.explanation,
                )
            )
    ...
    def _remove_container(self):
        if not self.postgresql_container:
            return
        logging.info(
            "Removing the docker container : {}-{}".format(
                self.postgresql_container.name, self.postgresql_container.id
            )
        )
        try:
    ...
    def _run_container(self, config: ComponentConfiguration):
        ...
        postgres_ports = {
            DEFAULT_CONTAINER_PORT: (DEFAULT_HOST_NAME, config.get_host_port())
        }
        ...
        self.postgresql_container = self.docker_client.containers.run(
            ...
            name=container_name,
            network=config.get_container_network(),
    ...
    def _write_secrets_to_file(self, db_username, db_password):
        ...
        db_password_path_ = self.secrets_path.joinpath(
            POSTGRES_PASSWORD_FILE_KEY
        ).resolve()
        try:
    ...
    def _create_config_command(self, config):
        ...
        for conf_file in server_configuration_files.keys():
            command = command + " -c {}={}".format(
                SUPPORTED_CONFIGURATION_FILES[conf_file], f"{CUSTOM_FILES}/{conf_file}"
            )
        return command

    def _follow_container_logs(self):
        def _follow_logs():
            logs_from_container = self.postgresql_container.logs(
